...
const IntroducePage: React.FC = () => {
  ...
  // 교회 비전 조회
  const { data: vision, isLoading: visionLoading } = useChurchInfo("vision");
  // 교회 소개 조회
  const { data: introduction, isLoading: introLoading } =
    useChurchInfo("introduction");
  // 교회 연혁 조회
  const { data: history, isLoading: historyLoading } = useChurchHistory();

  if (visionLoading || introLoading || historyLoading) {
    return (
      <SubMenuTemplate
        mainMenuTitle="교회소개"
          ...
          <S.SectionContent>
            {vision?.content ? (
              ...
              <p>
                ...
                사랑하고 섬기는 공동체를 지향합니다. 오늘보다 내일이 더 좋은
                교회로 성장하여 지역사회와 세상에 빛과 소금의 역할을 감당하고자
                합니다.
              </p>
            )}
          </S.SectionContent>
        ...
        <S.Section>
          ...
          <S.SectionContent>
            {introduction?.content ? (
              ...
              <>
                <p>
                  미래존교회는 성경을 기초로 한 믿음의 공동체로서, 하나님을
                  사랑하고 이웃을 사랑하는 그리스도인으로 양육하는 것을 목표로
                  합니다. 우리는 하나님의 형상으로 사람을 이해하고 사랑하며
                  도우며, 그리스도를 전하는 일에 힘쓰고 있습니다.
                </p>
                <p>
                  매주 예배와 함께 다양한 모임과 활동을 통해 성도들이 함께
                  성장하고 교제할 수 있는 공간을 제공하고 있습니다.
                </p>
              </>
            )}
          </S.SectionContent>
